...
class SalesReport(models.Model):
    ...
    m_sales_start_date = fields.Datetime(string="Start Date", default=datetime.today().replace(day=1), required=True, tracking=True)
    m_sales_end_date = fields.Datetime(string="End Date", default=datetime.today(), required=True, tracking=True)
    m_warehouse_id = fields.Many2many("stock.warehouse", string="Warehouse", required=True, tracking=True)
    m_exhausted = fields.Boolean("Include Exhausted Products", default=True, tracking=True)
    ...
    company_ids = fields.Many2many("res.company", string="Company", copy=False, tracking=True)
    product_ids = fields.Many2many("product.product", string="Products in Filter", copy=False, tracking=True)
    partner_ids = fields.Many2many("res.partner", string="Vendors in Filter", copy=False, tracking=True)
    internal_ref = fields.Char(string="Internal Reference in Filter", copy=False, tracking=True)
    prod_ref = fields.Char(string="Product Reference in Filter", copy=False, tracking=True)
    is_lock = fields.Boolean("Locked", default=False, copy=False, tracking=True)
    ...
    event_description = fields.Char(string="Event", copy=False, tracking=True)
    is_update_total_qty = fields.Boolean(default=False, copy=False, tracking=True)
    ...
    def _check_date(self):
        ...
        start_date = self.m_sales_start_date #datetime.strptime(self.m_sales_start_date, DEFAULT_SERVER_DATETIME_FORMAT)
        end_date = self.m_sales_end_date #datetime.strptime(self.m_sales_end_date, DEFAULT_SERVER_DATETIME_FORMAT)
    ...
    def clear_filter(self):
        ...
        Clear all fields related filters
        """
        self.update({'product_ids': False, 'partner_ids': False, 'internal_ref': ''})
        for rec in self:
            self.env.cr.execute("""UPDATE  sales_report_lines
                            SET active = TRUE
                            WHERE sales_report_id = %s"""% rec.id)
