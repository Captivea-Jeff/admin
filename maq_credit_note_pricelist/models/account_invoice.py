...
class AccountMove(models.Model):
    ...
    m_pricelist_id = fields.Many2one("product.pricelist", string='Pricelist',
                                     readonly=True, states={'draft': [('readonly', False)]})

    @api.constrains("m_pricelist_id", "currency_id")
    def _check_currency(self):
        for sel in self.filtered(lambda a: a.m_pricelist_id and a.is_invoice()):
            if sel.m_pricelist_id.currency_id != sel.currency_id:
                raise UserError(
    ...
    @api.onchange("m_pricelist_id")
    def _set_pricelist_currency(self):
        if (
                self.is_invoice()
                and self.m_pricelist_id
                and self.currency_id != self.m_pricelist_id.currency_id
        ):
            self.currency_id = self.m_pricelist_id.currency_id
    ...
    def _reverse_move_vals(self, default_values, cancel=True):
        ...
        if self.m_pricelist_id:
            move_vals["m_pricelist_id"] = self.m_pricelist_id.id
        return move_vals
...
class AccountMoveLine(models.Model):
    ...
    def _onchange_product_id_account_invoice_pricelist(self):
        for sel in self:
            if not sel.move_id.m_pricelist_id:
                return
    ...
    def _onchange_uom_id(self):
        for sel in self:
            if (
                ...
                and sel.move_id.m_pricelist_id
            ):
    ...
    def _get_real_price_currency(self, product, rule_id, qty, uom, m_pricelist_id):
        ...
        if rule_id:
            ...
            currency_id = pricelist_item.m_pricelist_id.currency_id
    ...
    def _get_price_with_pricelist(self):
        price_unit = 0.0
        if self.move_id.m_pricelist_id and self.product_id and self.move_id.is_invoice():
            if self.move_id.m_pricelist_id.discount_policy == "with_discount":
                product = self.product_id.with_context(
                    ...
                    pricelist=self.move_id.m_pricelist_id.id,
                ...
                final_price, rule_id = self.move_id.m_pricelist_id.with_context(
                ...
                base_price, currency = self.with_context(
                    ...
                    self.quantity,
                    self.product_uom_id,
                    self.move_id.m_pricelist_id.id,
                )
                if currency != self.move_id.m_pricelist_id.currency_id:
                    base_price = currency._convert(
                        base_price,
                        self.move_id.m_pricelist_id.currency_id,
    ...
    def _get_computed_price_unit(self):
        ...
        if self.move_id.m_pricelist_id and self.move_id.is_invoice():
