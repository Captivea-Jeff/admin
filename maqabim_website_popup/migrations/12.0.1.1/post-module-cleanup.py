...
def migrate(cr, version):
    ...
    cr.execute("update ir_ui_view set active = False where id in (select res_id from ir_model_data where module = 'website_multi');")
    cr.execute("update ir_ui_view set active = False where id in (1990,1543);")
