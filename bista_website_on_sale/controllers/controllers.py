...
class WebsiteSale(WebsiteSale):
    ...
    def sale(self, page=0, category=None, search='', min_price=0.0, max_price=0.0, ppg=False, **post):
        ...
        keep = QueryURL('/sale', category=category and int(category), search=search, attrib=attrib_list, min_price=min_price, max_price=max_price, order=post.get('order'))
        ...
        if node_field == 'sales_pricelist' or node_field == 'publish_date':
            product_list_values = self._company_dependent_order_by_sale(company_id, Product, categs, domain, sale_product_id, url, page, ppg, post)
